...
<div class="box">
    <div class="box-header">
        <form method="POST" action="{{ route("$controller.search") }}">
            ...
            <div class="col-xs-1" style="width:150px;">
                <label>伺服器IP</label>
                <input type="text" name="server_ip" class="form-control" placeholder="请输入..." value="{{ $search['server_ip'] ?? '' }}">
            </div>
        ...
        <table class="table table-hover">
            <thead>
                <tr>
                    ...
                    <th>{!! sort_title('name', '节点名称', $route, $order, $search) !!}</th>
                    <th>检测状态</th>
                    <th>{!! sort_title('server_ip', '伺服器IP', $route, $order, $search) !!}</th>
...
<script>
    ...
    function detects(id) {
        layer.open({
            ...
            offset: ['20px', ''],
            area: ['90%', '90%'],
            content: '{{ route("detect.index",["sidebar"=>0]) }}&node_id='+id
        });
    }
</script>
@endsection
